...
chrome_driver = "C:\\Users\\Administrator\\AppData\\Local\\Programs\\Python\\Python38\\chromedriver.exe"   # Win32_76.0.3809.126
...
def submitOrder(driver, user):
    ...
    while True:
        try:
            submitOrder = driver.find_element_by_link_text('支付订金')
            submitOrder.click()
            print(user + ':成功提交订单')
            break
        except:
...
def goToBuy(driver, user):
    ...
    while True:
        if time.time() > timestamp:  # 到了抢购时间
            ...
            text = driver.find_elements_by_xpath('//*[@id="pro-operation"]/a/span').text
            if text == '已售完':
                over = True
                break
